...
func scanTools(root string) []ToolEntry {
	...
	// Fallback for when root is already OlympusForge
	if _, err := os.Stat(filepath.Join(root, "81000-Toolchain-External")); err == nil {
		areas = []struct {
			path string
			cat  string
		}{
			{filepath.Join(root, "81000-Toolchain-External"), "external"},
			{filepath.Join(root, "82000-Toolchain-Fleet"), "fleet"},
			{filepath.Join(root, "90000-Enablement-Labs/000-Tools"), "internal"},
		}
	}
